...
override_prompt_active=$(jq -r '.overridePromptActive // false' "$STATE")
now_epoch=$(date +%s)

# --- An override timer that has finished waiting fires the passage prompt
# on its own, in a DETACHED background process. This has to be detached:
# launchd only runs one instance of this poller at a time, so if the prompt
# blocked this invocation directly, enforcement would freeze for as long as
# the prompt sat unanswered — a full bypass window. Backgrounding it keeps
# every subsequent poll (every ~5s) free to keep killing relaunch attempts
# while the prompt is up. ---
if [ "$override_pending_since" -gt 0 ] && [ "$override_prompt_active" != "true" ]; then
  elapsed=$((now_epoch - override_pending_since))
  if [ "$elapsed" -ge "$OVERRIDE_WAIT_SECONDS" ]; then
    log "Override wait complete — showing passage prompt (background)."
    write_state '.overridePendingSince = 0 | .overridePromptActive = true'
    (
      ...
      if [[ "$result" == OVERRIDE:* ]] && [ "$(normalize "${result#OVERRIDE:}")" = "$(normalize "$passage")" ]; then
        write_state '.bonusGames = ((.bonusGames // 0) + 1) | .locked = false | .wasRunningWhileLocked = false | .overridePromptActive = false'
        ...
        write_state '.overridePromptActive = false'
        kill_all_league
...
if [ "$locked" = "true" ]; then
  if any_league_running; then
    ...
    if [ "$was_notified" != "true" ]; then
      ...
      if [ "$override_prompt_active" = "true" ]; then
        osascript -e "display dialog \"Still locked. A passage prompt is already open somewhere on screen — answer it to unlock.\" with title \"League Limiter\" buttons {\"OK\"} default button \"OK\" with icon caution" >/dev/null 2>&1
      elif [ "$override_pending_since" -gt 0 ]; then
