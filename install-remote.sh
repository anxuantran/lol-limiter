...
#   curl -fsSL https://raw.githubusercontent.com/anxuantran/lol-limiter/main/install-remote.sh | bash -s -- 5
#
# Pass a daily limit as the first arg, same as install.sh.

set -euo pipefail

REPO="anxuantran/lol-limiter"
...
SRC_DIR="$(find "$TMP_DIR" -maxdepth 1 -type d -name "lol-limiter-*" | head -1)"
if [ -z "$SRC_DIR" ]; then
